...
plotProfile <- function(p, data) {
    ...
    if (length(reC)) {
        ...
        pdf(paste0("./Pdf/", ex, "_nextclade.pdf"), width = 7, height = 4)
    ...
    if (length(reL)) {
        ...
        pdf(paste0("./Pdf/", ex, "_pangolin.pdf"), width = 7, height = 4)
...
plotProfileMutation <- function(p, data) {
    ...
    if (length(re)) {
        ...
        pdf(paste0("./Pdf/", ex, "_mutation.pdf"), width = 7, height = 4)
...
metadata  <- read.csv("datafreeze-2022-07-16_12_weeks_good.tsv", sep = "\t")
...
##-------------------------------------------------##
nextclade <- read.delim("datafreeze-2022-07-16_12_weeks_good.nextclade.2.3.0.tsv", sep = "\t")[, 1:2]
colnames(nextclade) <- c("Isolate", "Clade")

pangolin  <- read.delim("datafreeze-2022-07-16_12_weeks_good.pangolin-4.1.2-UShER-.csv", sep = ",")[, 1:2]
...
write.table(glm.signif, file = "./Tables/significantChangesInVariants.tsv", sep = "\t", row.names = FALSE)


##---------------------------------------##
##-- logistic regression for mutations --##
##---------------------------------------##
mutation <- fromJSON(file("./mkolisko/report_fixed.json"))
...
write.table(glm.signif, file = "./Tables/significantChangesInMutations.tsv", sep = "\t", row.names = FALSE)
